    ...
    extends Specification
{
  ...
  def 'it retrieves proprietary config followed by remote scan followed by evaluation in correct order (happy path)'() {
    setup:
      def buildStep = new IqPolicyEvaluatorBuildStep("stage", ApplicationSelectType.applicationSelectTypeIfNullFactory(null, "appId"),"appId","", [new ScanPattern("*.jar")], [], false, null)
  ...
  def 'it expands environment variables for scan pattern'() {
    setup:
      def remoteScanner = Mock(RemoteScanner)
      def buildStep = new IqPolicyEvaluatorBuildStep("stage", ApplicationSelectType.applicationSelectTypeIfNullFactory(null, "appId"), "appId", "", [new ScanPattern('/path1/$SCAN_PATTERN/path2/')],
          [], false, "131-cred")
  ...
  def 'it ignores when no environment variables set for scan pattern'() {
    setup:
      def remoteScanner = Mock(RemoteScanner)
      def buildStep = new IqPolicyEvaluatorBuildStep("stage", ApplicationSelectType.applicationSelectTypeIfNullFactory(null, "appId"), "appId", "",
  ...
  def 'exception handling (part 1)'() {
    setup:
      ...
      def buildStep = new IqPolicyEvaluatorBuildStep('stage', ApplicationSelectType.applicationSelectTypeIfNullFactory(null, 'appId'), 'appId', '', [new ScanPattern('*.jar')], [],
  ...
  def 'it throws an exception when remote scanner fails'() {
    setup:
      ...
      def buildStep = new IqPolicyEvaluatorBuildStep('stage', ApplicationSelectType.applicationSelectTypeIfNullFactory(null, 'appId'), 'appId', '', [new ScanPattern('*.jar')], [], false,
          '131-cred')
  ...
  def 'global no credentials are passed to the client builder when no job credentials provided'() {
    setup:
      def buildStep = new IqPolicyEvaluatorBuildStep('stage', ApplicationSelectType.applicationSelectTypeIfNullFactory(null, 'appId'), 'appId', '', [new ScanPattern('*.jar')], [], true,
          jobCredentials)
